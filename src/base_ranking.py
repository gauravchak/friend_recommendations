...
class CrossLayer(nn.Module):
    ...
    def forward(self, x0, x):
        x = x0 * (torch.sum(x * self.weight, dim=-1, keepdim=True) + self.bias) + x
        return x
...
class MMoE(nn.Module):
    ...
    def __init__(self, input_dim, num_experts, num_tasks, expert_dim, hidden_dims):
        ...
        self.experts = nn.ModuleList([Expert(input_dim, expert_dim, hidden_dims) for _ in range(num_experts)])
    ...
    def forward(self, x):
        ...
        for task in range(self.num_tasks):
            gate_output = F.softmax(self.gates[task](x), dim=1)  # [batch_size, num_experts]
            gated_output = torch.sum(gate_output.unsqueeze(-1) * expert_outputs, dim=1)  # [batch_size, expert_dim]
...
class EncoderModel(nn.Module):
    ...
    def __init__(
        ...
        self.dcn = nn.ModuleList([CrossLayer(input_dim) for _ in range(dcn_layers)])
        self.mmoe = MMoE(input_dim, num_experts, n_tasks, expert_dim, expert_hidden_dims)
...
model = EncoderModel(
    user_dim=user_dim, n_tasks=n_tasks,
    dcn_layers=3, num_experts=4
)
